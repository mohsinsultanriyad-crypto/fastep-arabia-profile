...
const ContactPage = () => {
  return (
    <section className="brochure-page bg-background p-12 md:p-16 flex flex-col justify-center print:break-after-page">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <p className="text-sm font-heading font-semibold uppercase tracking-widest text-primary mb-8">Advanced Security & Surveillance Solutions</p>
            <div className="w-full h-px bg-border mb-8" />
            <p className="text-sm text-muted-foreground font-body leading-relaxed">
              For inquiries, technical consultations, or partnership opportunities, contact our team. We are committed to delivering excellence in every project.
            </p>
          </div>
          <div className="mt-8 p-6 border border-border text-center">
            <div className="w-24 h-24 border-2 border-border mx-auto mb-3 flex items-center justify-center">
              <span className="text-xs font-heading uppercase tracking-widest text-muted-foreground">QR Code</span>
            </div>
            <p className="text-xs font-heading uppercase tracking-widest text-muted-foreground">Scan to visit our website</p>
          </div>
        </div>
      </div>

      <div className="mt-12 pt-6 border-t border-border text-center">
        <p className="text-xs font-body text-muted-foreground">© {new Date().getFullYear()} FASTEP ARABIA. All Rights Reserved.</p>
      </div>
    </section>
  );
};
